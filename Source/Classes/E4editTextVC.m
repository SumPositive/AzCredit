...
#import "Global.h"
#import "Entity.h"
#import "E4editTextVC.h"

@interface E4editTextVC (PrivateMethods)
- (void)viewDesign;
- (void)done:(id)sender;
@end

@implementation E4editTextVC
@synthesize Pe4;
@synthesize PiField;
...
- (void)viewWillAppear:(BOOL)animated 
{
	...
	switch (PiField) {
		case 0:
			MtextView.text = Pe4.zName;
			break;
		case 1:
			MtextView.text = Pe4.zNote;
			break;
		default:
			return;
			break;
...
- (void)done:(id)sender
{
	...
	switch (PiField) {
		...
		{
			NSError *err = nil;
			NSManagedObjectContext *contx = Pe4.managedObjectContext;
			// トリム（両端のスペース除去）　＜＜Load時に zNameで検索するから厳密にする＞＞
			NSString *zName = [str stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
			if ([zName length] <= 0) {
				UIAlertView *alert = [[[UIAlertView alloc] initWithTitle:NSLocalizedString(@"E4zNameLess",nil)
																 message:NSLocalizedString(@"E4zNameLessMsg",nil)
																delegate:nil 
													   cancelButtonTitle:nil 
													   otherButtonTitles:@"OK", nil] autorelease];
				[alert show];
			...
			[request setPredicate:[NSPredicate predicateWithFormat:@"(%K != %ld) AND (%K = %@)", 
													@"nRow", [Pe4.nRow longValue], @"zName", zName]];
			// コンテキストにリクエストを送る
			NSArray* aRes = [contx executeFetchRequest:request error:&err];
			[request release];
			if (0 < [aRes count]) {
				UIAlertView *alert = [[[UIAlertView alloc] initWithTitle:NSLocalizedString(@"E4zNameDups",nil)
																 message:NSLocalizedString(@"E4zNameDupsMsg",nil)
																delegate:nil 
													   cancelButtonTitle:nil 
													   otherButtonTitles:@"OK", nil] autorelease];
				[alert show];
