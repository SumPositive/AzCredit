...
- (void)dealloc 
{
	...
	// @property (retain)
	AzRETAIN_CHECK(@"GooDocs Pe1selected", Pe1selected, 1)
	[Pe1selected release];
	AzRETAIN_CHECK(@"GooDocs PmanagedObjectContext", PmanagedObjectContext, 4)
...
- (GDataServiceGoogleDocs *)docsService {
	...
	if (!service) {
		...
		[service setUserAgent:@"Azukid.com-AzPacking-0.1"]; // set this to yourName-appName-appVersion
...
//	[self.actionProgress release];
	...
	if (error == nil) {
		...
		// 成功アラート
		UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Uploaded Compleat!", @"アップロード成功")
														message:nil
													   delegate:self 
											  cancelButtonTitle:nil 
											  otherButtonTitles:@"OK", nil];
		...
		UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Upload Fail", @"アップロード失敗")
														message:NSLocalizedString(@"Please try again after waiting a little.", @"少し待ってから再実行してください")
													   delegate:self 
											  cancelButtonTitle:nil 
											  otherButtonTitles:@"OK", nil];
		[alert show];
...
- (void)uploadFile {
	...
	@try {
		...
		NSString *title = [self.Pe1selected.zName stringByAppendingString:@".AzPack"]; //GD_GDOCS_EXT];  // Gdocs表示名
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
	if (indexPath.section == 0) {
		...
		else return 35;  // Username, Password
	}
	...
	switch (indexPath.section) {
		case 0: // Login Section
			switch (indexPath.row) {
				case 0: // User name
					...
					cell.textLabel.text = NSLocalizedString(@"Username:", @"ユーザー名");
					return cell;
					break;
				case 1: // Password
					...
					cell.textLabel.text = NSLocalizedString(@"Password:", @"パスワード");
					return cell;
					break;
				case 2: // Login
		...
		case 1: // Upload Section
			if (self.PbUpload) {
				...
				cell.textLabel.text = self.Pe1selected.zName;
			}
			return cell;
			break;
...
/*				
				// Revision list
				GooDownloadView *view = [[GooDownloadView alloc] initWithStyle:UITableViewStylePlain];
				view.managedObjectContext  = self.managedObjectContext;  // csvWriteのため
				view.addRow = self.selectedRow;
				view.mDocsService = [self docsService];
				view.mDocBase = [mDocListFeed entryAtIndex:indexPath.row];
...
//	NSString *z3;
	...
	@try {
		...
		//str = @"[Item],,,row,name,spec,stock,need,weight,note\n";
		str = @"[Item],,,name,note,stock,need,weight,\n";  // rowは自動連番付与する
		[output writeData:[str dataUsingEncoding:enc allowLossyConversion:YES]];
...
//		[output writeData:[mstr dataUsingEncoding:enc allowLossyConversion:YES]];
		...
		str = [NSString stringWithFormat:@",\"%@\",\"%@\",\n", 
												[self writeStr:self.Pe1selected.zName],
												[self writeStr:self.Pe1selected.zNote]];
		AzLOG(@"E1> %@", str);
		[output writeData:[str dataUsingEncoding:enc allowLossyConversion:YES]];
		
		//------------------------------------------------------------------------------E2 [Group]
		NSMutableArray *e2list = [[NSMutableArray alloc] initWithArray:[self.Pe1selected.invoices allObjects]];
		[e2list sortUsingDescriptors:sortDescriptors];	// .row 昇順にCSV書き出す
		for (E2invoice *e2obj in e2list) {

			[mstr setString:@""];
			[mstr appendFormat:@",,\"%@\",\"%@\",\n",	[self writeStr:e2obj.zName], 
														[self writeStr:e2obj.zNote]];
			AzLOG(@"E2> %@", mstr);
			[output writeData:[mstr dataUsingEncoding:enc allowLossyConversion:YES]];

			//----------------------------------------------------------------------------E3 [Item]
			NSMutableArray *e3list = [[NSMutableArray alloc] initWithArray:[e2obj.details allObjects]];
			[e3list sortUsingDescriptors:sortDescriptors];	// .row 昇順にCSV書き出す
			for (E3detail *e3obj in e3list) {

				// [Item] ,,,name,spec,note,need,weight,
				[mstr setString:@""];
				[mstr appendFormat:@",,,\"%@\",\"%@\",%ld,%ld,%ld,\n", 
													[self writeStr:e3obj.zName], 
													[self writeStr:e3obj.zNote], 
													[e3obj.stock longValue], 
													[e3obj.need longValue], 
	...
	@try {
		while (1) {
			...
			if (iSection==0 && [[csvArray objectAtIndex:0] isEqualToString:GD_PRODUCTNAME]
				...
				// [Item],,,name,note,stock,need,weight,
				//-----------------------------------------------E3
				...
				NSInteger iRequired = [e3obj.need intValue];
				NSInteger iWeight = [e3obj.weight intValue];
				e3obj.weightStk = [NSNumber numberWithInteger:(iWeight * iStock)];
				e3obj.weightNed = [NSNumber numberWithInteger:(iWeight * iRequired)];
