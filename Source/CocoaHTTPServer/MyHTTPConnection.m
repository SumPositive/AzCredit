...
{
//    NSArray *array = [[NSFileManager defaultManager] directoryContentsAtPath:path]; // is deprecated により廃止＆移行
	...
	[outdata appendFormat:@"<title>AzCredit Service %@</title>", server.name];
	...
	// AzCredit では、バックアップ／リストアを同画面で処理する
	[outdata appendString:NSLocalizedString(@"HTML Title",nil)];
	...
	while ((fname = [dirEnum nextObject]))
	{
		...
		//} 
		//else
		if ([fname isEqualToString:GD_CSVFILENAME]) {
			NSError *error = nil;
...
- (NSObject<HTTPResponse> *)httpResponseForMethod:(NSString *)method URI:(NSString *)path
{
	...
	{
		NSLog(@"processing post data: %ld", (long)requestContentLength);
	...
	//	}
		...
		// ダウンロード成功
		// CSV読み込み
		NSString *zErr = [FileCsv zLoad:server.Pe0root
					  fromLocalFileName:GD_CSVFILENAME]; // この間、待たされるのが問題になるかも！！
		if (zErr) {
